...
import { GET_PROJECTS_BY_ORG, CREATE_PROJECT, DELETE_PROJECT } from '@/graphql/mutations/projects';
...
const Projects = () => {
  ...
  const { data, loading, refetch } = useQuery(GET_PROJECTS_BY_ORG, {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) return;
    try {
      await createProject({ variables: { input: { name, description, organizationId: orgId } } });
            ...
            <DialogContent>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Mobile App v2" required />
                </div>
                <div className="space-y-2">
        ...
        {loading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((project: any) => (
              <Card key={project.id} className="group border-border transition-shadow hover:shadow-lg">
