...
const TeamMembersList = ({ teamId }: { teamId: string }) => {
  ...
  return (
    <div className="space-y-2">
      {members.map((m: any) => (
        <div key={m.id} className="flex items-center justify-between rounded-md bg-background p-2">
...
const Teams = () => {
  ...
  const { data, loading, refetch } = useQuery(GET_TEAMS_BY_ORG, {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) return;
    try {
      await createTeam({ variables: { input: { name, organizationId: orgId } } });
            ...
            <DialogContent>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Frontend Team" required />
                </div>
                <Button type="submit" className="w-full gold-gradient text-primary-foreground" disabled={creating}>
        ...
        {loading ? (
          ...
          <Card className="border-border">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-muted-foreground">Create your first team to organize members</p>
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {teams.map((team: any) => (
              <Card key={team.id} className="group border-border transition-shadow hover:shadow-lg">
                ...
                {expandedTeam === team.id && (
                  <CardContent className="border-t border-border pt-4">
                    <h5 className="mb-2 text-sm font-medium text-foreground">Members</h5>
